  ...
  pro.forEach((e) => {
    ...
    if (findKey?.[1]) {
      ...
      if (nm) {
        ...
        // 随机生成地区
        const randomIndex = Math.floor(Math.random() * outList.length);
        const randomRegion = outList[randomIndex];
        let keyover = [],
          usflag = "";
        if (addflag) {
          usflag = FG[randomIndex];
          usflag = usflag === "🇹🇼" ? "🇨🇳" : usflag;
        }
        keyover = keyover
          .concat(firstName, usflag, nNames, randomRegion, retainKey, ikey, ikeys)
          .filter((k) => k !== "");
        e.name = keyover.join(FGF);
      }
    }
